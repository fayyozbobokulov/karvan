# GCP Birthdate REST -- /gcp/birthdaterest/v1
...
This endpoint provides passport data retrieval by document number (passport series + number) and birth date. It is used when both the document identifier and birth date are known but the PINFL is not available. Returns the standard GCP passport response and applies the same normalization as other passport category endpoints.

---

## Methods

### `passport_by_document_birthdate`

| Field         | Value                   |
| ------------- | ----------------------- |
| HTTP Method   | POST                    |
| Endpoint      | `/gcp/birthdaterest/v1` |
| Service       | egov_main               |
| Timeout       | 60000ms                 |
| Requires Auth | Yes                     |
...
```typescript
{
  methodName: "passport_by_document_birthdate",
