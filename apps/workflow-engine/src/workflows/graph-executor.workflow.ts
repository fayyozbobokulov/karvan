...
export async function executeFlowGraph(input: FlowInput) {
  ...
  async function executeNode(nodeId: string): Promise<void> {
    ...
    const unitDef = await loadUnitDefinition({ unitId: node.unit });
    ...
    const unitInstanceId = await createUnitInstance({
      flowInstanceId,
    ...
    switch (unitDef.type) {
      case 'DOCUMENT': {
        const docResult = await executeDocumentUnit({
          unitInstanceId,
          flowInstanceId,
          config: mergedConfig,
          context: ctx,
  ...
  try {
    ...
    if (
      ...
      await createNotification({
        ...
        type: overallStatus === 'completed' ? 'flow_completed' : 'flow_failed',
        title:
          overallStatus === 'completed'
